...
> **Before starting this step:** Read `POLARIS_DESIGN_SYSTEM.md` in full. All color choices, spacing values, typography sizes, component patterns, and dark mode configuration are defined there. Do not use default shadcn/ui styling — override with the Polaris theme variables.

### 4.1 Apply Theme Configuration

Before creating any pages, apply the CSS variables from `POLARIS_DESIGN_SYSTEM.md` §6.1 to `src/app/globals.css`. Install `next-themes` and wrap the root layout in `<ThemeProvider>` per §5.1. This must happen first so all subsequent components inherit the correct theme.
